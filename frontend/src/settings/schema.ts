  ...
  performance: {
    ...
    items: {
      ...
      perHostDelay: {
        ...
        help: "Minimum ms between request starts to the same host. This is the throughput ceiling for a single-host crawl: max pages/sec ≈ 1000/this value (e.g. 500ms → 2/s), since starts are serialized regardless of concurrency. Pair with the max below for a randomized range — fresh draw per request defeats interval-regularity bot detectors (DataDome, PerimeterX, Akamai). Raise toward 2000ms+ if a cookie-gated site blocks you; 0 disables.",
      },
      perHostDelayMax: {
        type: "number",
        default: 1500,
        ...
        help: "Upper end of the randomized range. Sidecar samples a uniform random value in [min, max] before each request. MUST be ≥ min, or jitter is silently disabled and the limiter runs at fixed min. Wider range = harder to fingerprint as a bot but slower aggregate throughput.",
      },
      perHostConcurrency: {
        type: "number",
        default: 4,
        ...
        help: "Maximum concurrent requests to the same host. Typical real-user concurrency is 2–6. Only raises throughput when responses are slower than the per-host delay (otherwise the delay gate binds first). Natural response-time variance gives you organic concurrency jitter for free, so this stays a single max (no range). NOTE: headed mode forces concurrency to 1 regardless of this setting (single visible browser tab).",
      },
      navTimeout: {
        type: "number",
        default: 10000,
        ...
        help: "Max time to wait for a page (page.goto, DOMContentLoaded) before recording it as a status-0 error. A timed-out page holds a host-concurrency slot for the full duration, so on slow/heavy sites a lower value frees the slot faster and lifts throughput. Default 10000ms; raise it if legitimately slow pages are timing out.",
      },
      sessionWarmup: {
        type: "boolean",
        default: true,
