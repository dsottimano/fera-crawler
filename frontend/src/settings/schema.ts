  ...
  performance: {
    ...
    items: {
      ...
      perHostDelay: {
        ...
        help: "Minimum ms between request starts to the same host. Pair with the max below for a randomized range — fresh draw per request defeats interval-regularity bot detectors (DataDome, PerimeterX, Akamai). Default 2000ms is the polite-crawler floor; adaptive walls reliably block faster pacing on cookie-gated sites. 0 disables.",
      },
      perHostDelayMax: {
        type: "number",
        default: 5000,
        ...
        help: "Upper end of the randomized range. Sidecar samples a uniform random value in [min, max] before each request. Set equal to min for fixed delay (no jitter). Wider range = harder to fingerprint as a bot but slower aggregate throughput. 2000–5000ms is a safe-by-default range.",
      },
      perHostConcurrency: {
        type: "number",
        default: 2,
        ...
        help: "Maximum concurrent requests to the same host. Typical real-user concurrency is 2–4. Natural response-time variance gives you organic concurrency jitter for free, so this stays a single max (no range). NOTE: headed mode forces concurrency to 1 regardless of this setting (single visible browser tab).",
      },
      sessionWarmup: {
        type: "boolean",
        default: true,
